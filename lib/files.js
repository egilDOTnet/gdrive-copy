...
function Files(properties, Drive) {
  this.Drive = Drive;
  ...
  this.getFiles = listFiles(this.Drive);

  this._buildErrorResult = buildResult(true);
  this._buildSuccessResult = buildResult(false, null);
  this.copyFileToDrive = copyFile(this.Drive);
  this.copyFolderToDrive = copyFolder(this.Drive);
  this.createLoggerSpreadsheet = copyLoggerSpreadsheet(this.Drive);
  this.initializeDestinationFolder = insertDestinationFolder(this.Drive);
...
/////////////////////////////
//
// Define basic database functions (copy, insert, list)
//
/////////////////////////////
var copyFile = R.curry(function(Drive, file) {
  try {
    var newItem = Drive.Files.copy(this._arrangeRequestBody(file), file.id);

    return this._buildSuccessResult(newItem.id, newItem.title);
  } catch (err) {
    // TODO: incorporate better error handling for user Rate Limit Exceeded
    // return this.handleCopyItemError(err, file);
    return this._buildErrorResult(err.message, item.id, item.title);
  }
});

var copyFolder = R.curry(function(Drive, folder) {
  try {
 ...
 */
var listFiles = R.curry(function(Drive, query) {
 ...
 */
var copyLoggerSpreadsheet = R.curry(function(Drive, today, destId) {
  try {
 ...
 * 
 * @param {object} Drive - Google Drive service
 * @param {object} selectedFolder - srcName
 * @param {string} today - a string representation of today
 * @return {Object} metadata for destination folder, or error on failure
 */
var insertDestinationFolder = R.curry(function(
  Drive,
  DriveApp,
  selectedFolder,
  today
  ...
  if (this._properties.shouldCopyPermissions()) {
    Permissions.prototype.copyPermissions(
...
/////////////////////////////
//
// Define primary prototype functions
//
/////////////////////////////
...
Files.prototype.copyItem = function(item) {
  ...
  if (file.mimeType === 'application/vnd.google-apps.folder') {
    return this.copyFolderToDrive(item);
  }
  return this.copyFileToDrive(item);
};
 ...
 */
Files.prototype.createPropertiesDocument = function(DriveApp, destId) {
  try {
...
Files.prototype.getSpreadsheet = function() {
  return SpreadsheetApp.openById(
    this._properties.getServiceProperty('spreadsheetId')
  ).getSheetByName('Log');
};
    ...
    parents: [
      {
        /*
                04/01/2017
                instead of hardcoding "drive#fileLink" for the parents.kind prop,
                use the kind from the item's parent
                */
        kind: item.parents[0].kind,
...
var buildResult = R.curry(function(isError, errMessage, id, title) {
  return {
    id: id,
    title: title,
